...
import { 
  getCarteiraComparativa, 
  getCarteiraIdeal,
  upsertCarteiraIdeal,
  deleteCarteiraIdeal,
  inicializarCarteiraIdeal
...
export function CarteiraIdealManager() {
  ...
  const fetchData = async () => {
    try {
      setIsLoading(true);
      const [comparativaData, idealData] = await Promise.all([
        getCarteiraComparativa(),
        getCarteiraIdeal()
      ]);
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-4">
            {carteiraComparativa.map((item, index) => (
              <div key={item.tipo || index} className="flex items-center justify-between p-4 border rounded-lg">
                ...
                <div className="flex items-center gap-3">
                  <div className="text-right">
                    <div className={`text-sm font-medium ${
                      (item.diferenca_percentual || 0) > 0 ? 'text-green-600' : 
                      (item.diferenca_percentual || 0) < 0 ? 'text-red-600' : 'text-gray-600'
                    }`}>
                  ...
                  {editingId === item.tipo ? (
                    <div className="flex items-center gap-2">
                      ...
                      <Button 
                        size="sm" 
                        onClick={() => handleSave(item.tipo!, parseFloat(editValue))}
                      >
                        Salvar
                      </Button>
                      <Button 
      ...
      {carteiraIdeal.length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle>Itens da Carteira Ideal</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-2">
              {carteiraIdeal.map((item) => (
                <div key={item.id} className="flex items-center justify-between p-3 border rounded">
                  <div>
                    <span className="font-medium">{item.tipo}</span>
                    <span className="ml-2 text-muted-foreground">{item.percentual_ideal}%</span>
                  </div>
                  <Button
                    size="sm"
                    variant="outline"
